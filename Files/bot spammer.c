...
int main()
{
    ...
    read_hotkey_file(hotkey_file, &opt, &vk_code);
    switch(opt) {
        case 1:
            RegisterHotKey(NULL, 1, MOD_CONTROL | MOD_NOREPEAT, vk_code);
            break;
        case 2:
            RegisterHotKey(NULL, 1, MOD_ALT | MOD_NOREPEAT, vk_code);
            break;
        case 3:
            RegisterHotKey(NULL, 1, MOD_SHIFT | MOD_NOREPEAT, vk_code);
            break;
        case 4:
            RegisterHotKey(NULL, 1, MOD_WIN, vk_code);
            break;
        default:
            break;
    }
    while (TRUE)
    {
        ...
        case 'F':
            ...
            case '1':
            case 'A':
                hotkey_ctrl_reg(hotkey_file);
                break;
            case '2':
            case 'B':
                hotkey_alt_reg(hotkey_file);
                break;
            case '3':
            case 'C':
                hotkey_shift_reg(hotkey_file);
                break;
            case '4':
            case 'D':
                hotkey_win_reg(hotkey_file);
                break;
            default:
                break;
            }
